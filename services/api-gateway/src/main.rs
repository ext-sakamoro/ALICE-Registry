...
/// ヘルスチェックエンドポイント。ステータスとアップタイムを返す。
...
async fn license_handler() -> (HeaderMap, Json<LicenseInfo>) {
    ...
    (
        ...
        Json(LicenseInfo {
            ...
            source_code: "https://github.com/ext-sakamoro/ALICE-Registry".into(),
...
/// 認証ミドルウェア。Bearer JWT または X-API-Key を検証する。
///
/// # Errors
///
/// - `401 UNAUTHORIZED` — トークンが無効、またはヘッダーが未指定の場合。
async fn auth_mw(
...
/// レートリミットミドルウェア。ユーザーごとにトークンバケットで制御する。
///
/// # Errors
///
/// - `429 TOO_MANY_REQUESTS` — レートリミット超過時。
async fn rate_mw(
    State(s): State<Arc<AppState>>,
    req: Request,
    next: Next,
    ...
    let uid = req
        .extensions()
        .get::<Claims>()
        .map_or_else(|| "anon".into(), |c| c.sub.clone());
    let ok = {
        let mut e = s
            .rate_limiters
            .entry(uid)
...
/// リクエストを上流サービスに転送するリバースプロキシ。
///
/// # Errors
///
/// - `400 BAD_REQUEST` — リクエストボディの読み込み失敗。
/// - `502 BAD_GATEWAY` — 上流サービスへの接続失敗またはレスポンス読み込み失敗。
/// - `500 INTERNAL_SERVER_ERROR` — レスポンス構築失敗。
